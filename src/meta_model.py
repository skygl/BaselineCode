...
class BertNER(BertForTokenClassification):
    ...
    def __init__(self, config, support_per_class, cuda_device, use_bias=True, use_global=False,
    ...
    def direct_forward(self, input_ids,
        ...
        if metric == 'dp':
            ...
            if self.use_bias:
                query_sets_copy = query_sets.unsqueeze(-2).repeat(1, 1, support_class_num * 2,
                                                                  1)  # .view(-1,self.support_class_num * 2,feat_dim)
            else:
                query_sets_copy = query_sets.unsqueeze(-2).repeat(1, 1, support_class_num * 2 + 1, 1)
            # del query_sets
            logits = - torch.sum(torch.pow(query_sets_copy - prototypes, 2), dim=3)
        ...
        if self.use_bias:
            logits = torch.cat((self.background.unsqueeze(0).unsqueeze(0).repeat(len(labels), max_len, 1), logits),
                               dim=2)
        # for k in range(len(logits[0][0])):
        #     print(f"{k} dot product {logits[0][0][k]}")
        ...
        if not soft_kmeans:
            ...
            for cls in range(len(embed_class_len)):
                if embed_class_len[cls] == 0:
                    ...
                    prototypes1[cls] = (0.1 * query_scores[cls] + embed_class_len[cls] * prototypes[cls]) / (
                                0.1 * sum_per_cls[cls] + embed_class_len[cls])
    ...
    def direct_forward_unsup(self, input_ids,
        ...
        if t_prob is not None:
            kl_criterion = torch.nn.KLDivLoss()
            loss = kl_criterion(torch.log(F.softmax(logits.view(-1, support_class_num * 2 + 1), dim=-1)),
                                F.softmax(t_prob.view(-1, support_class_num * 2 + 1), dim=-1))
            # print(outputs[0][:10])
            # print(F.softmax(logits.view(-1, support_class_num * 2 + 1), dim=-1))
            # print(F.softmax(t_prob.view(-1, support_class_num * 2 + 1), dim=-1))
            # print(f"loss {loss}")
